...
class RefreshJWTTokensUseCase:
    def __init__(self, token_service: TokenServiceABC):
        self._token_service = token_service

    async def __call__(self, refresh_token: str) -> TokenPair:
        try:
            data = self._token_service.decode_jwt_token(refresh_token)
        except:
            raise InvalidTokenError

        new_access_token = self._token_service.create_jwt_token(
            data=data, expires=settings.jwt_config.access_token_expire
        )
        new_refresh_token = self._token_service.create_jwt_token(
            data=data, expires=settings.jwt_config.refresh_token_expire
        )
